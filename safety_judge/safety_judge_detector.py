...
=========================================
将训练好的 DeBERTa-v3-base Safety Judge 集成到 RefusalGuard 的
双通道检测器（M2）中，作为高精度语义分类通道。

使用方法（在 RefusalGuard 的 detector.py 中引入）：
    from backend.modules.safety_judge_detector import safety_judge

    result = safety_judge.predict(prompt)
    # result.label      : 0=benign / 1=injection / 2=harmful
    # result.confidence : 最高类别的置信度 (0~1)
    # result.scores     : [benign_prob, injection_prob, harmful_prob]
    # result.available  : 模型是否已加载
"""
...
class SafetyJudge:
    """
    Safety Judge 推理器（单例模式）。
    首次调用 predict() 时懒加载模型，避免启动时阻塞。
    """
